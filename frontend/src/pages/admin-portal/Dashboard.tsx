...
const STATS = [
  { title: 'Total Users',       value: 47,    icon: '👥', accent: 'indigo' as const, trend: '+3 this week',  trendType: 'up'   as const },
  { title: 'Active Students',   value: 1_204, icon: '🎓', accent: 'blue'   as const, trend: '+18 this term', trendType: 'up'   as const },
  { title: 'Pending Approvals', value: 2,     icon: '⏳', accent: 'amber'  as const, trend: 'Action needed', trendType: 'down' as const },
  { title: 'Support Tickets',   value: 1,     icon: '🎫', accent: 'red'    as const, trend: 'Open',          trendType: 'down' as const },
  { title: 'Fee Collection',    value: 'UGX 77M', icon: '💰', accent: 'green' as const, trend: '92% of target', trendType: 'up' as const },
  { title: 'System Uptime',     value: '99.9%',   icon: '🟢', accent: 'teal'  as const, trend: '30-day avg',    trendType: 'up' as const },
];

const RECENT_ACTIVITY = [
  { user: 'Ms. Nakakande Mary',  action: 'Logged in',              role: 'Teacher',        time: '2 min ago',  status: 'info'    },
  { user: 'Mr. Kato Emmanuel',   action: 'Updated fee record',     role: 'Bursar',         time: '18 min ago', status: 'success' },
  { user: 'Sr. Nakamya Rose',    action: 'Added sick bay visit',   role: 'Nurse',          time: '45 min ago', status: 'info'    },
  { user: 'Unknown',             action: 'Failed login attempt',   role: 'Unknown',        time: '1 hr ago',   status: 'danger'  },
  { user: 'Mr. Ssali Brian',     action: 'Created sport fixture',  role: 'ECA Officer',    time: '2 hrs ago',  status: 'info'    },
  { user: 'Mrs. Namukasa Joyce', action: 'Filed welfare report',   role: 'Counsellor',     time: '3 hrs ago',  status: 'info'    },
];
          ...
          {tab === 'activity' && (
            <div>
              {RECENT_ACTIVITY.map((a, i) => (
                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '9px 0', borderBottom: '1px solid #f1f5f9' }}>
                  <div style={{
                  ...
                  <div style={{ flex: 1 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#1e293b' }}>{a.user}</div>
                    <div style={{ fontSize: 11, color: '#64748b' }}>{a.action} · <span style={{ color: '#94a3b8' }}>{a.role}</span></div>
                  </div>
                  <div style={{ textAlign: 'right' }}>
                    <Badge variant={a.status as any} size="sm">{a.status === 'danger' ? '⚠️ Alert' : '✓'}</Badge>
          ...
          {tab === 'portals' && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 10 }}>
              {PORTAL_SUMMARY.map(p => (
                <div key={p.label} onClick={() => navigate(p.path)} style={{
          ...
          <Card>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {[
                ...
                { label: '📋 Approve Pending (2)',    path: '/admin/approvals'   },
